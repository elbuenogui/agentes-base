...
4. [Faixa de áudio com histórico + botão de cancelar] — pedido do usuário em 2026-08-20, a partir
   ...
   > **A resolver ao gerar a tarefa:** "só pausa" quer dizer interromper de vez (encerrar a sessão,
   > como o botão de parar faz hoje, só que sem enviar) ou pausar de forma retomável (manter a
   > conexão aberta e voltar de onde parou)? Perguntar ao usuário antes de escrever a tarefa — muda
   > bastante a implementação no modo tempo real.
